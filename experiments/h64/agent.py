...
def _use_local_exact_route(query: str) -> bool:
    lowered = query.lower()
    exact_cues = ("when", "where", "year", "date", "born", "died", "\"")
    return any(cue in lowered for cue in exact_cues) or bool(re.search(r"\b\d{4}\b", lowered))
...
async def retrieve(
    ...
    if _use_local_exact_route(query):
