...
async def _bridge_queries(query: str) -> list[str]:
    prompt = (
        "Break this question into two short retrieval queries that expose a bridge fact.\n"
        "Return the two queries separated by newlines only.\n\n"
        f"Question: {query}"
    )
    try:
        resp = await _openai.chat.completions.create(
            model=MODEL,
            messages=[
                {"role": "system", "content": "You create bridge retrieval queries."},
                {"role": "user", "content": prompt},
            ],
        )
        lines = [line.strip("- ").strip() for line in (resp.choices[0].message.content or "").splitlines()]
...
async def retrieve(
    ...
    if _use_rich_retrieval(query):
        retrieval_queries = await _bridge_queries(query)
        candidate_k = max(top_k * 3, top_k)
    try:
        ranked_lists: list[list[dict[str, Any]]] = []
        for retrieval_query in retrieval_queries:
            vector = (await asyncio.to_thread(embed_batch, [retrieval_query]))[0]
            ranked_lists.append(_es_vector_search(idx, vector, candidate_k))
            ranked_lists.append(_es_text_search(idx, retrieval_query, candidate_k))
        hits = _fuse_hits_rrf(*ranked_lists, k=top_k)
    except Exception:
        try:
