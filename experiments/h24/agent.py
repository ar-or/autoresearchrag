...
async def _run_agent(
    ...
    if contexts:
        compression_prompt = (
            "Compress the retrieved documents into only the facts needed to answer the question.\n"
            "Keep citations as [n] references and preserve important numbers and qualifiers.\n\n"
            f"Question: {user_message}\n\n"
            f"Retrieved documents:\n{_format_context_block(contexts)}"
        )
        compression_resp = await _chat_once(
            [
                {"role": "system", "content": SYSTEM_PROMPT},
                {"role": "user", "content": compression_prompt},
            ]
        )
        _merge_usage(usage, _usage_from_response(compression_resp))
        compressed_context = (compression_resp.choices[0].message.content or "").strip()
