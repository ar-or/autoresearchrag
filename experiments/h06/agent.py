"""H06 experiment: expand strong hits with adjacent chunks from the same document."""
...
def _expand_with_neighbors(index: str, hits: list[dict[str, Any]]) -> list[dict[str, Any]]:
    expanded = list(hits)
    for hit in hits[:2]:
        src = hit.get("_source", {})
...
async def retrieve(
    ...
    try:
        ...
        hits = _expand_with_neighbors(idx, fused_hits)
    except Exception:
        try:
