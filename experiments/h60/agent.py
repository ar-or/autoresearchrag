...
async def _revise_unsupported_answer(
    question: str,
    answer: str,
    contexts: list[RetrievedContext],
) -> str:
    prompt = (
        ...
        "If it is supported, return it unchanged.\n"
        "If it is unsupported or incomplete, revise it to a short evidence-grounded answer.\n"
    ...
    try:
        resp = await _openai.chat.completions.create(
            model=MODEL,
            messages=[
                {"role": "system", "content": "You revise answers to stay grounded in evidence."},
                {"role": "user", "content": prompt},
            ],
        )
...
async def _run_agent(
    ...
    for _ in range(max_iterations):
        ...
        content = await _revise_unsupported_answer(user_message, content, contexts)
