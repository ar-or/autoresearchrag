...
PAGE_CHUNK_SPAN = int(os.environ.get("PAGE_CHUNK_SPAN", "4"))
RESERVED_INDEX_FIELDS = {"text", "chunk_index", "hash_id", "embedding"}


def ensure_hash_id_mapping():
    """Add missing metadata fields to the existing index mapping if needed."""
    ...
    missing = {name: mapping for name, mapping in wanted.items() if name not in props}
    if not missing:
        return
...
def summarize_page(chunks: list[str]) -> str:
    ...
    for chunk in chunks:
        parts = re.split(r"(?<=[.!?])\s+|\n+", chunk)
        for part in parts:
            cleaned = normalize_chunk_text(part)
            if cleaned:
...
def prepare_document(
    ...
    page_records = _build_page_records(chunks)
    return {
        "hash_key": hash_key,
...
def _build_page_records(chunks: list[str]) -> list[dict]:
    page_records: list[dict] = []
    for start in range(0, len(chunks), PAGE_CHUNK_SPAN):
        page_chunks = chunks[start : start + PAGE_CHUNK_SPAN]
        if not page_chunks:
            continue
        summary = summarize_page(page_chunks)
        if not summary:
            continue
        page_records.append(
            {
                ...
                "text": summary,
                "child_chunk_indexes": list(range(start, start + len(page_chunks))),
            }
        )
    return page_records
...
def _prepare_batch_records(documents: list[dict]) -> list[dict]:
    ...
    for document in documents:
        ...
        for chunk_index, chunk in enumerate(document.get("chunks", [])):
            ...
            page_number = (chunk_index // PAGE_CHUNK_SPAN) + 1
            record = {
                "chunk_index": chunk_index,
                "text": chunk,
                "hash_id": hash_id,
                "fields": {**fields, "node_type": "passage", "page_number": page_number},
            }
            chunk_records.append(record)
            records.append(record)

        for page_record in document.get("page_records", []):
            child_hash_ids = [
                chunk_records[idx]["hash_id"]
                for idx in page_record.get("child_chunk_indexes", [])
                if idx < len(chunk_records)
            ]
            if not child_hash_ids:
                continue

            page_hash_id = build_chunk_hash_id(
                f"{document_key}:page:{page_record['page_number']}",
                page_record["text"],
            )
            if page_hash_id in seen_in_batch:
                continue
            seen_in_batch.add(page_hash_id)
            records.append(
                {
                    "chunk_index": page_record["page_number"],
                    "text": page_record["text"],
                    "hash_id": page_hash_id,
                    "fields": {
                        **fields,
                        "node_type": "page",
                        "page_number": page_record["page_number"],
                        "child_hash_ids": child_hash_ids,
                    },
                }
            )
