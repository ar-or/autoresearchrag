...
async def _run_agent(
    ...
    seen_queries = {_normalized_query(user_message)}
        ...
        if choice.finish_reason == "tool_calls" and choice.message.tool_calls:
            ...
            for tc in choice.message.tool_calls:
                ...
                if normalized in seen_queries:
                    query = await _rewrite_retrieval_query(
                        f"{query} focus on missing bridge entity or comparison target"
                    )
                    args["query"] = query
                    normalized = _normalized_query(query)
                seen_queries.add(normalized)
                rewritten_arguments = json.dumps(args)
                print(f"    [tool_call] {tc.function.name}({rewritten_arguments})")
                tool_output, tool_contexts = await _handle_tool_call(
                    tc.function.name, rewritten_arguments
                )
