...
def _answer_spans(answer: str) -> list[str]:
    spans = [part.strip() for part in re.split(r",|;|\band\b", answer) if part.strip()]
    return spans or [answer]


async def _repair_answer_spans(
    question: str,
    answer: str,
    contexts: list[RetrievedContext],
) -> str:
    evidence = "\n\n".join(
        f"[{index+1}] {(context.title + ': ') if context.title else ''}{context.text[:300]}"
        for index, context in enumerate(contexts[:5])
    )
    prompt = (
        "Check each answer span against the evidence and rewrite only if any span is unsupported.\n"
        "Return only the repaired answer text.\n\n"
        f"Question: {question}\n"
        f"Answer spans: {json.dumps(_answer_spans(answer))}\n\nEvidence:\n{evidence}"
    )
    try:
        resp = await _openai.chat.completions.create(
            model=MODEL,
            messages=[
                {"role": "system", "content": "You repair unsupported answer spans."},
                {"role": "user", "content": prompt},
            ],
        )
...
async def _run_agent(
    ...
    for _ in range(max_iterations):
        ...
        content = await _repair_answer_spans(user_message, content, contexts)
