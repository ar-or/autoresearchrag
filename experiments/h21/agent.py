...
async def _run_agent(
    ...
    forecast_prompt = user_message
    if contexts:
        forecast_prompt = (
            "Retrieved documents:\n"
            f"{_format_context_block(contexts)}\n\n"
            f"Question: {user_message}\n\n"
            "Forecast the next single sentence you expect a grounded answer would need to say."
        )
    forecast_resp = await _chat_once(
        [
            {"role": "system", "content": SYSTEM_PROMPT},
            {"role": "user", "content": forecast_prompt},
        ]
    )
    _merge_usage(usage, _usage_from_response(forecast_resp))
    forecast_query = (forecast_resp.choices[0].message.content or "").strip()
    if forecast_query:
        contexts = _dedupe_contexts(contexts + await retrieve(forecast_query), RETRIEVAL_K)
