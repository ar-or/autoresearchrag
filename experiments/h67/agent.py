...
def _question_taxonomy_terms(query: str) -> str:
    lowered = query.lower()
    labels: list[str] = []
    if lowered.startswith("who"):
        labels.extend(["person", "identity"])
    if lowered.startswith("when") or re.search(r"\b\d{4}\b", lowered):
        labels.extend(["date", "year", "time"])
    if lowered.startswith("where"):
        labels.extend(["place", "location"])
    if "both" in lowered or "and" in lowered:
...
async def retrieve(
    ...
    taxonomy_query = f"{retrieval_query} {_question_taxonomy_terms(query)}".strip()
    try:
    ...
    try:
        return await _sentence_rerank_contexts(taxonomy_query, contexts, top_k)
    except Exception:
        return contexts[:top_k]
