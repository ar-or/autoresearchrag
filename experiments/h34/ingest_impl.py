...
def ensure_hierarchical_mapping():
    """Add sentence-retrieval fields if needed."""
    r = requests.get(f"{ELASTIC_URL}/{ELASTIC_INDEX}/_mapping")
    r.raise_for_status()
    props = r.json().get(ELASTIC_INDEX, {}).get("mappings", {}).get("properties", {})
    ...
    for field_name, field_type in (
        ...
        if field_name not in props:
            missing[field_name] = {"type": field_type}
    if not missing:
        return
...
def chunk_text(text: str, chunk_size: int = CHUNK_SIZE, overlap: int = CHUNK_OVERLAP) -> list[str]:
    """Split text on structural boundaries before falling back to windows."""
    ...
    for unit in structural_units:
        if len(unit) > chunk_size:
            if current_parts:
                ...
                current_parts = []
                current_length = 0
            chunks.extend(_window_chunk(unit, chunk_size, overlap))
            continue

        projected = current_length + len(unit) + (1 if current_parts else 0)
            ...
            records.append(
                {
                    ...
                    "text": chunk,
                    "hash_id": hash_id,
                    "fields": dict(fields, record_type="chunk"),
                }
            )

            sentence_texts = _split_sentences(chunk)
            for sentence_index, sentence in enumerate(sentence_texts):
                sentence_hash = hashlib.sha256(
                    f"{hash_id}\n{sentence_index}\n{normalize_chunk_text(sentence)}".encode()
                ).hexdigest()
                if sentence_hash in seen_in_batch:
                    continue
                seen_in_batch.add(sentence_hash)
                records.append(
                    {
                        "chunk_index": chunk_index,
                        "sentence_index": sentence_index,
                        "text": sentence,
                        "hash_id": sentence_hash,
                        "fields": dict(
                            fields,
                            record_type="sentence",
                            parent_chunk_id=hash_id,
                            parent_chunk_text=chunk,
                        ),
                    }
                )
...
def _index_with_embeddings_batch(records: list[dict], embeddings: list[list[float]]) -> tuple[int, int]:
    ...
    for record, emb in zip(records, embeddings):
        ...
        if "sentence_index" in record:
            body["sentence_index"] = record["sentence_index"]
