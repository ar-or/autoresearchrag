...
async def _premium_rerank_contexts(
    query: str,
    contexts: list[RetrievedContext],
) -> list[RetrievedContext]:
    if not contexts:
        return []
    prompt = (
        "Select the most useful passages for answering the question.\n"
        "Return JSON only with key keep containing the best passage numbers in ranked order.\n\n"
        f"Question: {query}\n\nCandidates:\n"
        + "\n\n".join(
            f"{index+1}. title={context.title}\ntext={context.text[:320]}"
            for index, context in enumerate(contexts[:10])
        )
    )
    try:
        resp = await _openai.chat.completions.create(
            model=MODEL,
            messages=[
                {"role": "system", "content": "You rerank first-hop retrieval evidence."},
                {"role": "user", "content": prompt},
            ],
        )
        keep = _parse_json_object(resp.choices[0].message.content or "").get("keep", [])
        ...
        if chosen:
            return chosen[:RETRIEVAL_K]
    except Exception:
        pass
    return contexts[:RETRIEVAL_K]
...
async def _run_agent(
    ...
    contexts = await retrieve(user_message, k=max(RETRIEVAL_K * 2, 10))
    contexts = await _premium_rerank_contexts(user_message, contexts)
