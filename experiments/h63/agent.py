...
async def _logic_tree_queries(question: str) -> list[str]:
    prompt = (
        "Decompose the question into up to two supporting subquestions plus one final synthesis query.\n"
        "Return one query per line and keep them short.\n\n"
        f"Question: {question}"
    )
    try:
        resp = await _openai.chat.completions.create(
            model=MODEL,
            messages=[
                {"role": "system", "content": "You decompose questions for retrieval."},
                {"role": "user", "content": prompt},
            ],
        )
        queries = [line.strip("- ").strip() for line in (resp.choices[0].message.content or "").splitlines()]
        return [query for query in queries if query][:3] or [question]
    except Exception:
        return [question]
...
async def _run_agent(
    ...
    queries = await _logic_tree_queries(user_message)
    contexts: list[RetrievedContext] = []
    seen: set[tuple[str, str]] = set()
    for query in queries:
        for context in await retrieve(query):
