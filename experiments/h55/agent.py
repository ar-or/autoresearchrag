...
async def _llm_rerank_hits(
    ...
    for index, hit in enumerate(hits[:12], start=1):
        ...
        text = src.get("text") or src.get("content") or src.get("pageContent", "")
        candidates.append(
            f"{index}. title={src.get('title', '')}\ntext={text[:320]}"
        )
    prompt = (
        "Rank the candidate passages for answering the question.\n"
        "Return JSON only with key keep containing the best candidate numbers in ranked order.\n\n"
        f"Question: {query}\n\nCandidates:\n" + "\n\n".join(candidates)
    )
    try:
        resp = await _openai.chat.completions.create(
            model=MODEL,
            messages=[
                {"role": "system", "content": "You rerank passages for retrieval."},
                {"role": "user", "content": prompt},
            ],
        )
...
async def retrieve(
    ...
    try:
        ...
        hits = await _llm_rerank_hits(retrieval_query, text_hits, top_k)
    except Exception:
        try:
