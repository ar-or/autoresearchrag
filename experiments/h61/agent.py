...
async def _self_refine_answer(
    question: str,
    answer: str,
    contexts: list[RetrievedContext],
) -> str:
    evidence = "\n\n".join(
        f"[{index+1}] {(context.title + ': ') if context.title else ''}{context.text[:320]}"
        for index, context in enumerate(contexts[:5])
    )
    try:
        feedback_resp = await _openai.chat.completions.create(
            model=MODEL,
            messages=[
                {"role": "system", "content": "You give concise feedback on QA drafts."},
                {
                    "role": "user",
                    "content": (
                        "Give one short line of feedback about omissions, contradictions, or unsupported claims.\n\n"
                        f"Question: {question}\nDraft answer: {answer}\n\nEvidence:\n{evidence}"
                    ),
                },
            ],
        )
        feedback = (feedback_resp.choices[0].message.content or "").strip()
        rewrite_resp = await _openai.chat.completions.create(
            model=MODEL,
            messages=[
                {"role": "system", "content": "You refine answers using concise feedback."},
                {
                    "role": "user",
                    "content": (
                        "Rewrite the answer using the feedback while staying grounded in the evidence. "
        ...
        revised = (rewrite_resp.choices[0].message.content or "").strip()
...
async def _run_agent(
    ...
    for _ in range(max_iterations):
        ...
        content = await _self_refine_answer(user_message, content, contexts)
