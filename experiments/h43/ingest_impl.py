...
RAPTOR_CHILDREN_PER_SUMMARY = int(os.environ.get("RAPTOR_CHILDREN_PER_SUMMARY", "4"))
...
def _summary_sentence(chunk: str) -> str:
    parts = re.split(r"(?<=[.!?])\s+|\n+", chunk.strip())
    return parts[0].strip() if parts and parts[0].strip() else chunk.strip()[:160]


def _build_raptor_records(chunks: list[str], document_fields: dict, document_key: str) -> list[dict]:
    records: list[dict] = []
    leaf_hash_ids: list[str] = []
    for chunk_index, chunk in enumerate(chunks):
        hash_id = build_chunk_hash_id(f"{document_key}:leaf", chunk)
        leaf_hash_ids.append(hash_id)
        fields = dict(document_fields)
        fields["node_type"] = "leaf"
        fields["tree_level"] = 0
        records.append(
            {
    ...
    for group_index, start in enumerate(range(0, len(chunks), RAPTOR_CHILDREN_PER_SUMMARY)):
        child_chunks = chunks[start : start + RAPTOR_CHILDREN_PER_SUMMARY]
        child_ids = leaf_hash_ids[start : start + RAPTOR_CHILDREN_PER_SUMMARY]
        summary_text = " ".join(_summary_sentence(chunk) for chunk in child_chunks if chunk.strip())
        if not summary_text.strip():
            continue
        hash_id = build_chunk_hash_id(f"{document_key}:summary:{group_index}", summary_text)
        fields = dict(document_fields)
        fields["node_type"] = "summary"
        fields["tree_level"] = 1
        fields["child_hash_ids"] = child_ids
        records.append(
            {
                "chunk_index": len(chunks) + group_index,
...
def prepare_document_chunks(
    ...
    return {
        ...
        "fields": document_fields,
        "chunks": chunks,
        "raptor_records": _build_raptor_records(chunks, document_fields, hash_key),
    }
...
def _prepare_batch_records(documents: list[dict]) -> list[dict]:
    ...
    for document in documents:
        ...
        raptor_records = document.get("raptor_records")
        if raptor_records:
            candidate_records = raptor_records
        else:
            candidate_records = []
            for chunk_index, chunk in enumerate(document.get("chunks", [])):
                ...
                item_fields["node_type"] = "leaf"
                item_fields["tree_level"] = 0
